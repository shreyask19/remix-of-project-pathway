...
const StudentOverview = () => {
  ...
  const recentActivity = [
    { icon: <CheckCircle className="w-4 h-4 text-success" />, text: "Submitted Payment API project", time: "2 hours ago" },
    { icon: <Zap className="w-4 h-4 text-warning" />, text: "Earned 15 credits from Stripe challenge", time: "Yesterday" },
        ...
        {stats.map((stat) => (
          <div key={stat.label} className={`glass-card bg-gradient-to-br ${stat.gradient} hover:-translate-y-0.5 transition-transform`}>
            <div className="flex items-start justify-between mb-4">
              <div className="w-10 h-10 rounded-2xl bg-background/80 backdrop-blur text-primary flex items-center justify-center shadow-sm">
                {stat.icon}
              </div>
              ...
              {upcomingDeadlines.map((deadline, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-secondary/50">
                  ...
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                    deadline.daysLeft <= 3 ? "bg-destructive/10 text-destructive" : "bg-warning/10 text-warning"
                  }`}>
